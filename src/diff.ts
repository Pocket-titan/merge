    ...
    if ((lenA && lenB) || lenA > 3 || lenB > 3) {
      ...
      if (!lenA || !lenB) {
        ...
        if (lenBefore && lenAfter) {
          ...
          !lenAfter &&
          (indentLen =
            change.fromA - (indentBefore = findIndentBefore(a, change.fromA, maxScanBefore))) &&
...
/// appropriate.
export function oldPresentableDiff(a: string, b: string, config?: DiffConfig): readonly Change[] {
...
export function presentableDiff(a: string, b: string, config?: DiffConfig): readonly Change[] {
  ...
  const aText = Text.of(a.split(/\r?\n/));
  const bText = Text.of(b.split(/\r?\n/));

  const diffComputer = new DiffComputer(
    Array.from(aText.iterLines()),
    Array.from(bText.iterLines()),
    options
  );
  ...
  const results = changes
    .reduce((acc, change) => {
      if (!change.charChanges) {
        ...
        const [fromA, fromB] = [
          aText.line(originalStartLineNumber).from,
          bText.line(modifiedStartLineNumber).from,
        ];

        let [toA, toB] = [
          originalEndLineNumber === 0 ? fromA : aText.line(originalEndLineNumber).to,
          modifiedEndLineNumber === 0 ? fromB : bText.line(modifiedEndLineNumber).to,
        ];
      ...
      (change.charChanges || []).forEach(
        ({
          ...
          acc.push({
            fromA: aText.line(originalStartLineNumber).from + originalStartColumn - 1,
            toA: aText.line(originalEndLineNumber).from + originalEndColumn - 1,
            fromB: bText.line(modifiedStartLineNumber).from + modifiedStartColumn - 1,
            toB: bText.line(modifiedEndLineNumber).from + modifiedEndColumn - 1,
          });
        }
      );

      return acc;
    }, [] as { fromA: number; toA: number; fromB: number; toB: number }[])
    .map((x) => new Change(x.fromA, x.toA, x.fromB, x.toB));
