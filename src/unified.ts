...
export function unifiedMergeView(config: UnifiedMergeConfig) {
  let orig =
    typeof config.original == "string" ? Text.of(config.original.split(/\r?\n/)) : config.original;
...
const originalDoc = StateField.define<Text>({
  create: () => Text.empty,
  update(doc, tr) {
    const isEditable = tr.startState.facet(EditorView.editable);
    const isReadOnly = tr.startState.readOnly;
...
const deletedChunks = StateField.define<DecorationSet>({
  ...
  update(deco, tr) {
    return tr.state.field(ChunkField, false) != tr.startState.field(ChunkField, false)
